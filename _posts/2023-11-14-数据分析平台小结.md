...
可以看到比较的轻型且容易理解，没有任何中间件，是一个典型的用Flink+TiDB实现的流批一体、实时计算的OLAP架构。
...
![2](https://i.postimg.cc/5ycqXjFN/69-DAA5-AA-39-E0-466b-82-F6-3-FBC49-C24890.png)
...
本项目中，只是将source和sink换成了TiDB source、TiDB Sink(TiDB有完整的Binlog)，加上insert into query的Flink SQL，就完成了Flink+TiDB快速构建实时OLAP架构的数据仓库。
