...
```html
		...
		,SUM(t.OnlineDelayCount) as OnlineDelayCount
		,SUM(t.OfflineDelayCount) as OfflineDelayCount
		,SUM(t.PersonAffairCount) as PersonAffairCount
		,SUM(t.LegalAffairCount) as LegalAffairCount
		,SUM(t.UnacceptedOvertimeCount) as UnacceptedOvertimeCount
		,SUM(t.ApplyCount) as ApplyCount
		,SUM(t.UnacceptedCount) as UnacceptedCount
		,SUM(t.AcceptedCount) as AcceptedCount
		,SUM(t.AcceptedOvertimeCount) as AcceptedOvertimeCount
		,SUM(t.UndoAffairCount) as UndoAffairCount
		,SUM(t.UndoOvertimeCount) as UndoOvertimeCount
		,SUM(t.EndAffairCount) as EndAffairCount
		,SUM(t.EndOntimeCount) as EndOntimeCount
		...
		(
			SELECT  
					...
					SUM(case when a.ApplyType='0' then 1 ELSE 0 end) as OfflineCount,
					SUM(case when a.ApplyType='1' and a.IsOvertime=1 then 1 ELSE 0 end) as OnlineDelayCount,
					SUM(case when a.ApplyType='0' and a.IsOvertime=1 then 1 ELSE 0 end) as OfflineDelayCount,
					SUM(case when date(a.Receiveuserdate) is null then 1 ELSE 0 end) UnacceptedCount,		
					SUM(case when a.ApplyerType="自然人" then 1 ELSE 0 end) PersonAffairCount,
					SUM(case when a.ApplyerType="法人" then 1 ELSE 0 end) LegalAffairCount,
					...
					0 AS AcceptedCount,
					SUM(case when a.isAcceptOverTime=1 and date(a.Receiveuserdate) is not null then 1 ELSE 0 end) as  AcceptedOvertimeCount, 		
					0 as UndoAffairCount,
					0 as UndoOvertimeCount,
					0 as EndAffairCount,
					0 as EndOntimeCount,
					0 as EndOvertimeCount
				FROM ryb_dwd.dwd_que_project_info a 
				INNER JOIN 	
					(
						SELECT date(b.ApplyerDate) as ApplyerDate, B.EventId
						FROM ryb_dwd.dwd_que_project_info b 
				...
				SELECT  
					...
					0 AS OnlineCount,
					0 AS OfflineCount,
					0 as OnlineDelayCount,
					0 as OfflineDelayCount,
					0 as UnacceptedCount,
					0 as PersonAffairCount,
					0 as LegalAffairCount,
					0 as UnacceptedOvertimeCount,
					0 as ApplyCount,
