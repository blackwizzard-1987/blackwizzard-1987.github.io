...
![8](https://i.postimg.cc/4yTjwHz3/9.png)

这里因为**上游集群所有节点日志的连续性不可控**加上没有场景去进行Checkpoint、Savepoint的恢复（基本任务无法解决这类问题），所以在长时间内这个问题是很麻烦的，每次切换之后都需要响应告警，然后将受影响的任务重新部署，根据受影响的面和每个任务的情况进行评估，对中断任务期间遗漏的数据进行修补，耗费大量人力。
