...
如果有多个相同结构的where条件，只是关键词不同，比如为一个数组的不确定个数的元素，那么可以用foreach collection完成where条件和case when的判断。

```html
	 kw_list = []
	 for (item in kwList){
		 kw_list.push(item.get('wordName'))
	 }
-- 效果为多个关键字满足同一结构后，合并所有关键字keyword的结果
	<if test="kw_list != null and kw_list.size() > 0">
	AND (
		<foreach collection="kw_list" item="keyword" separator=" OR ">
			(
			b.business_scope like concat('%', #{keyword}, '%')
			and b.business_scope not like concat('%不含[（\\s]*', #{keyword}, '[）\\s;。,%]*%')  
			and b.business_scope not like concat('%不含', #{keyword}, '%')
			and b.business_scope not like concat('%不含（', #{keyword}, '）%')
			and b.business_scope not like concat('%不含 ', #{keyword}, '%')
			and b.business_scope not like concat('%不含', #{keyword}, '；%')
			and b.business_scope not like concat('%不含', #{keyword}, '。%')
...
-- case when同理，循环取关键词数组中的关键词然后拼接(这里假设关键词之间是互斥的)
	<if test="kw_list != null and kw_list.size() > 0">
	CONCAT( 
		<foreach collection="kw_list" item="keyword" separator=",">
			CASE WHEN 
			(
			b.business_scope like concat('%', #{keyword}, '%')
			and b.business_scope not like concat('%不含[（\\s]*', #{keyword}, '[）\\s;。,%]*%')  
			...
			THEN #{keyword} ELSE '' END
		</foreach>
	) as key_word,
	</if>
	<if test="kw_list == null or kw_list.size() == 0">
		'' as key_word, 
	</if>
```
